...
app.get('/api/data', (req, res) => {
    const filePath = path.join(__dirname, 'sinavlar2.csv');

    if (!fs.existsSync(filePath)) {
        console.error('HATA: sinavlar2.csv dosyası sunucuda bulunamadı!');
        return res.status(404).json({ error: 'Veri dosyası (sinavlar2.csv) bulunamadı.' });
    }
    ...
    fs.createReadStream(filePath)
        .pipe(csv({
            ...
            .on('data', (data) => {
                ...
                if (data.TARİH && data.TARİH.trim() && !data.TARİH.startsWith('-')) {
                    results.push(data);
                }
            })
            .on('end', () => {
